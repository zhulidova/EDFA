...
f(1:N.s,1) = 2 * pi * n_sum * trapz(r,((repmat(sigma.es',1,N_r)-repmat(sigma.esa_etaC.*sigma.as',1,N_r)) .* repmat(n.second,N.s,1)...
    - repmat(sigma.as',1,N_r) .* repmat(n.first,N.s,1) - repmat(sigma.bgs',1,N_r)).* repmat(P(1:N.s,1),1,N_r) .* psi.ns .* repmat(r,N.s,1),2);

% уравнения для попутного ASE (dP_ase/dz)
f(N.s+1:N.s+N.ase,1)  = 2 * pi * n_sum * (trapz(r,((repmat(sigma.ease',1,N_r)-repmat(sigma.esa_etaC.*sigma.aase',1,N_r)) .* repmat(n.second,N.ase,1)...
    ...
    repmat(P(N.s+1:N.s+N.ase,1),1,N_r) .* repmat(sigma.bgase',1,N_r)) .* psi.nase .* repmat(r,N.ase,1),2));

for i = 1: length(Lambda.pf)
    if Lambda.pf(i) > 1450* 10^(-9)
        % уравнения для попутной накачки (dP_p/dz) на длине волны 1480 нм
        f(N.s+N.ase+i,1)  = 2 * pi * n_sum * (trapz(r,((sigma.epf(i)-sigma.apf(i)*sigma.esa_etaC) .* n.second(1,:) - sigma.apf(i) .* n.first(1,:) - sigma.bgpf(i))...
            .* P(N.s+N.ase+i) .*  psi.npf(i,:) .* r(1,:)));
    elseif Lambda.pf(i) < 1100* 10^(-9)
        % уравнения для попутной накачки (dP_p/dz) на длине волны 980 нм
        f(N.s+N.ase+i,1)  = 2 * pi * n_sum * (trapz(r,((sigma.epf(i)-sigma.apf(i)*sigma.esa_eta980) .* n.third(1,:) - sigma.apf(i) .* n.first(1,:) - sigma.bgpf(i)) .* P(N.s+N.ase+i) .*...
            psi.npf(i,:) .* r(1,:)));
    end
end
...
if isempty(Lambda.pb) == 0
    ...
    f(N.s+N.ase+N.pf+1:N.s+N.ase+N.pf+N.ase,1) = -2 * pi * n_sum * (trapz(r,((repmat(sigma.ease',1,N_r)-repmat(sigma.esa_etaC.*sigma.aase',1,N_r)) .*...
        ...
        repmat(n.first,N.ase,1) .* repmat(P(N.s+N.ase+N.pf+1:N.s+N.ase+N.pf+N.ase,1),1,N_r) - repmat(P(N.s+N.ase+N.pf+1:N.s+N.ase+N.pf+N.ase,1),1,N_r).*repmat(sigma.bgase',1,N_r))...
    ...
    for i = 1: length(Lambda.pb)
        if Lambda.pb(i) > 1450* 10^(-9)
            ...
            f(N.s+N.ase+N.pf+N.ase+i,1)  = -2 * pi * n_sum * (trapz(r,((sigma.epb(i)-sigma.apb(i)*sigma.esa_etaC) .* n.second(1,:) - sigma.apb(i) .* n.first(1,:) - sigma.bgpb(i))...
        ...
        elseif Lambda.pb(i) < 1100* 10^(-9)
            ...
            f(N.s+N.ase+N.pf+N.ase+i,1)  = -2 * pi * n_sum * (trapz(r,((sigma.epb(i)-sigma.apb(i)*sigma.esa_eta980) .* n.third(1,:) - sigma.apb(i) .* n.first(1,:) - sigma.bgpb(i)) .* P(N.s+N.ase+N.pf+N.ase+i) .*...
                psi.npb(i,:) .* r(1,:)));
        end
    end
end
