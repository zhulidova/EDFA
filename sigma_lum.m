...
CrossLum_smooth     = spline(param_em(:,1), CrossLum, lum_smooth);                        % сглаженный спектр 1              
CrossAbs_smooth     = spline(param_abs(:,1), CrossAbs, abs_smooth);       

sigma.es       = interp1(lum_smooth, CrossLum_smooth, Lambda.s * 10^(9));             % сечение люминесценции (сигнал)
sigma.as       = interp1(abs_smooth, CrossAbs_smooth, Lambda.s * 10^(9));            % сечение поглощения (сигнал)
sigma.apf      = interp1(abs_smooth, CrossAbs_smooth, Lambda.pf * 10^(9));           % сечение поглощения (попутная накачка)
sigma.ease     = interp1(lum_smooth, CrossLum_smooth, Lambda.ase * 10^(9));           % сечение люминесценции (ASE)
sigma.aase     = interp1(abs_smooth, CrossAbs_smooth, Lambda.ase * 10^(9));          % сечение поглощения (ASE)
sigma.apb      = interp1(abs_smooth, CrossAbs_smooth, Lambda.pb * 10^(9));           % сечение поглощения (встречная накачка)

sigma.esa_etaC    = 0.1;                   % отношение сечения перехода с 2 на 4 уровень к сечению перехода с 1 на 2 уровень, эффект ESA
...
for i = 1:length(Lambda.pf)
    if Lambda.pf(i) > 1450* 10^(-9)
        sigma.epf(i)  = interp1(lum_smooth, CrossLum_smooth, Lambda.pf(i) * 10^(9));            % сечение люминесценции (попутная накачка)
    else
        sigma.epf(i)  = 0;
    end
end
if isempty(Lambda.pb) == 0
    ...
    for i = 1:length(Lambda.pb)
        if min(Lambda.pb) > 1450* 10^(-9)
            sigma.epb(i)  = interp1(lum_smooth, CrossLum_smooth, Lambda.pb(i) * 10^(9));            % сечение люминесценции (встречная накачка)
        else
            sigma.epb(i)  = 0;
        end
    end
