...
CrossLum      = 1.3 * param(:,3) .* exp((-1) * 10^(-21) * param(:,2) / ph_const.k / T); % расчет сечения люминесценции

% сглаживание
wl_smooth     = linspace(min(param(:,1)), max(param(:,1)), 80);                        % новая сетка (80 точек длин волн)
...
avgCrossAbs   = spline(param(:,1), CrossAbs, wl_smooth2);                               % сглаженный спектр сечения       
