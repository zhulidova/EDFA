...
%% цикл для поиска недостающего условия для задачи Коши
     while abs(sum(lamda1) - sum(lamda)) > epsilon
         ...
         if low_gain_regime == 1 
            ...
            [z, P_out1]     = ode45(@(z,P) odu2(z, P, Lambda, sigma, psi, N, n_sum, ph_const, Pin, r_edf, N0), [0: L/N0.z: L],[Pin.s Pin.asef Pin.pf ksi1 lamda1]);  
            [z, P_out2]     = ode45(@(z,P) odu2(z, P, Lambda, sigma, psi, N, n_sum, ph_const, Pin, r_edf, N0), [0: L/N0.z: L],[Pin.s Pin.asef Pin.pf ksi2 lamda2]); 
         end
     ...
     if low_gain_regime == 1
        ...
        [z, P_out]  = ode45(@(z,P) odu2_unsaturated_gain_regime(z, P, Lambda, sigma, N,w_edf, n_sum, ph_const, Pin), [0: L/N0.z: L],[Pin.s Pin.asef Pin.pf ksi1 lamda1]);
         ...
         % решение подробной системы уравнений (общий случай)
        [z, P_out]  = ode45(@(z,P) odu2(z, P, Lambda, sigma, psi, N, n_sum, ph_const, Pin, r_edf, N0), [0: L/N0.z: L], [Pin.s Pin.asef Pin.pf ksi1 lamda1]);
     end
end