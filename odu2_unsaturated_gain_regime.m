function f = odu2_unsaturated_gain_regime(z, P, Lambda, sigma, N,w_edf, n_sum, ph_const, P_in,r_edf)
...
overlap.sum_as      = sum(p.s' .* sigma.as ./ (sigma.as + sigma.es)) +...       % слагаемое с усилением для сигнала и ASE
    sum(p.asef' .* sigma.aase ./ (sigma.aase + sigma.ease));                       % сумма всех приведенных мощностей
...
alpha.pb        = (1 - exp(-(r_edf ./ w_edf.pb).^2)) * n_sum .* sigma.apb;    % коэффицент истощения встречной накачки
end

overlap.es      = (overlap.sum_ap) ./ overlap.sum_all.^2 .* (overlap.sum_all...   % интеграл перекрытия (усиление) для сигнала
    .* (1 - exp(-(r_edf ./ w_edf.s).^2)));
overlap.as      = (overlap.sum_es + overlap.sum_ep) ./ overlap.sum_all.^2 .*... % интеграл перекрытия (поглощение) для сигнала
    (overlap.sum_all .* (1 - exp(-(r_edf ./ w_edf.s).^2)));
overlap.ease    = (overlap.sum_ap) ./ overlap.sum_all.^2 .* (overlap.sum_all .*...% интеграл перекрытия (усиление) для ASE
    (1 - exp(-(r_edf ./ w_edf.ase).^2)));
overlap.aase    = (overlap.sum_es + overlap.sum_ep) ./ overlap.sum_all.^2 .*... % интеграл перекрытия (поглощение) для ASE
    (overlap.sum_all .* (1 - exp(-(r_edf ./ w_edf.ase).^2)));
overlap.ep      = (overlap.sum_ap) ./ overlap.sum_all.^2 .* (overlap.sum_all...   % интеграл перекрытия (усиление) для сигнала
    .* (1 - exp(-(r_edf ./ w_edf.pf).^2)));
overlap.ap      = (overlap.sum_es + overlap.sum_ep) ./ overlap.sum_all.^2 .*... % интеграл перекрытия (поглощение) для сигнала
    (overlap.sum_all .* (1 - exp(-(r_edf ./ w_edf.pf).^2)));
...
g.ep            = n_sum .* sigma.epf .* overlap.ep;                              % диф. коэффициент усиления (сигнал)
g.ap            = n_sum .* sigma.apf .* overlap.ap;
...
f(N.s+N.ase+1:N.s+N.ase+N.pf,1)  = (g.ep - g.ap)' .* P(N.s+N.ase+1:N.s+N.ase+N.pf,1);
